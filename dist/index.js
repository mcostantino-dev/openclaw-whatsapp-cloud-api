...
const whatsappCloudChannel = {
    ...
    meta: {
        ...
        blurb: "WhatsApp via Meta's official Cloud API. Production-safe for business — no Baileys, no ban risk.",
    ...
    // ---- Config adapter ----
    config: {
        listAccountIds: (cfg) => cfg?.channels?.["whatsapp-cloud"]?.enabled !== false ? [DEFAULT_ACCOUNT_ID] : [],
        ...
        isConfigured: (account) => Boolean(account.config.accessToken?.trim() && account.config.phoneNumberId?.trim()),
        ...
        resolveAllowFrom: ({ cfg }) => (cfg?.channels?.["whatsapp-cloud"]?.allowFrom ?? []).map((entry) => String(entry)),
    ...
    security: {
        resolveDmPolicy: ({ cfg, accountId, account }) => ({
            ...
            normalizeEntry: (raw) => raw.replace(/[^0-9]/g, ""),
        }),
    },
    // ---- Pairing ----
    pairing: {
        idLabel: "whatsappPhoneNumber",
        normalizeAllowEntry: (entry) => entry.replace(/[^0-9]/g, ""),
    ...
    outbound: {
        ...
        sendText: async ({ cfg, to, text, accountId }) => {
            const config = resolveConfig(cfg);
            const log = getWhatsAppCloudRuntime()?.logging?.getChildLogger?.({ channel: "whatsapp-cloud" }) ?? console;
    ...
    gateway: {
        startAccount: async (ctx) => {
            ...
            webhookServer = startWebhookServer(config, 
...
const plugin = {
    ...
    register(api) {
        ...
        if (globalThis.__whatsappCloudApiRegistered) {
            ...
            api.registerCli(({ program }) => {
                ...
                cmd
                    ...
                    try {
                        ...
                        const result = await sendText(config, phone.replace("+", ""), "Hello from OpenClaw! Your WhatsApp Cloud API channel is working.", log);
                        if (result.ok) {
