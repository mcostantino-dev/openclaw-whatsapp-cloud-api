...
// Server
// ---------------------------------------------------------------------------
export function startWebhookServer(config, onMessage, onStatus, log) {
    ...
    server.listen(config.webhookPort, () => {
        log.info(`[whatsapp-cloud] Webhook server listening on port ${config.webhookPort} at ${config.webhookPath}`);
    });
    server.on("error", (err) => {
...
// ---------------------------------------------------------------------------
function handleVerification(url, config, res, log) {
...
// ---------------------------------------------------------------------------
async function handleIncoming(req, res, config, onMessage, onStatus, log) {
    // Read body
    let rawBody = "";
    ...
    if (config.appSecret) {
        ...
        log.debug("[whatsapp-cloud] No appSecret configured — skipping signature verification (NOT safe for production)");
    }
    // Parse and process
    let payload;
    try {
        ...
        for (const change of entry.changes) {
            ...
            if (errors?.length) {
                for (const err of errors) {
                    log.error(`[whatsapp-cloud] Webhook error ${err.code}: ${err.title} — ${err.message}`);
                }
            }
...
// ---------------------------------------------------------------------------
function processMessage(msg, contacts, config, onMessage, log) {
    // Access control
    ...
    log.info(`[whatsapp-cloud] ← ${senderName} (${msg.from}): ${inbound.text.slice(0, 100)}${inbound.text.length > 100 ? "…" : ""}`);
    // Send read receipt
